...
| 5 | EKF fusion | `SignalFusion` | `missing_sensor`, `outlier_rejected` | 当前 §5 evidence now includes radar + near-field fiducial updates; still synthetic and not production sensor proof |
...
| 8 | Bounded LS allocation | `WeightedLoadBalancer`, `CatchLoadAdapter` | `bounded_ls_residual` | residual 可观测但不会自动消失，需要业务告警语义 |
| + | Lyapunov stability | `StabilityGuard` | `stability_violation` | Includes `sre_error_budget_V` example; manual-reset latch remains explicit |
...
- `CatchController` 属于物理层；SRE event 由 `CatchLoadAdapter` wrapper 产生，不让 `starship/` 反向依赖 `sre_control/events.py`。
