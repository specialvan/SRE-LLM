...
- 当前文档同步目标中的 pytest 数量为 607; 规范输出行只在下方复核摘要中保留一次。
...
| `92fab09` | 将 StabilityGuard 布尔边界新增测试后的质量门目标同步到 589。 | 这是最近一轮前置基线, 不是当前最终 count; 当前 count 以 607 和现场 `quality_gate_counts` 输出为准。 |
...
`analysis/evidence_report.py` 对新模块的调用点, `tests/test_evidence_manifest.py` 中注入 S10 trace 语义漂移的用例, 以及
...
和 `contract_adapter_family_mismatch` 两个拆分后分支, 以现场 `git log -1 --oneline` 为准。
