...
- `scripts/quality_gate_counts.py` runs `python -m pytest tests`, parses the real pass summary, verifies it matches `pytest --collect-only -q`, runs installed-wheel smoke, updates only the current-facing quality-gate fields in `PR-REQUIREMENTS.md` and V2 HTML, and fails loudly on missing or duplicate matches.
