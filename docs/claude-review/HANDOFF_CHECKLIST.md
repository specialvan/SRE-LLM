...
- [ ] `git log --oneline -5` 最上面一条是 `41cdea8 review: align DEGRADED_GUARD + cover guardrail/allocator event propagation`
...
| 命令 | 预期 | 大致耗时 |
|---|---|---|
| `python -m pytest tests -q` | `33 passed` | ~1 s |
...
- [ ] 5 min · `EVENT_LIFECYCLE.md` 的 §3（三种场景的事件演化）
...
3. `claude-review/REVIEW_OF_CODEX_SESSION.md` ← 上一 session 的评审
4. `claude-review/DETAILED_ARCHITECTURE.md` ← 五视图架构
5. `claude-review/EVENT_LIFECYCLE.md` ← 事件时序
6. `claude-review/FAILURE_MODES.md` ← 失效与降级
7. `docs/EVENT_SCHEMA.md` ← Codex 上一轮写的 schema 指南
8. `docs/API_CONTRACTS.md` ← Codex 上一轮写的 API 契约
9. `docs/RUNTIME_STATES.md` ← Codex 上一轮写的状态机
10. `docs/knowledge-base.html` ← 主入口（可以放到浏览器看 GIF）

## 5. 本轮交接的三个 invariants（不能破坏）

### ✅ Invariant 1 · 依赖方向单向
- **`starship/` 不得 import `sre_control/`**
- 验证：`grep -rE "^(from|import) sre_control" starship/` 必须为空
...
- [小] `test_event_schema.py` 里 `"Do not" in counterexample` 字符串签名脆
- [小] `API_CONTRACTS.md §2.9 CatchController` 放在 SRE 文档里容易误导
- [小] `ARCHITECTURE.md` 与 `CODEX_HANDOFF.md` 都提到 CatchController wrapper 建议

来自 `DETAILED_ARCHITECTURE.md §8 演化蓝图`：

- [中] 加 `starship/stability_monitor.py`（§2.1 Lyapunov）
- [中] 加 `tests/test_import_graph.py` 护栏住依赖方向
...
**最近一个 human reviewer 做的事（`41cdea8`）**：
