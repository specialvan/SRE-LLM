...
| `tests/test_contracts.py` | 13 | runtime degraded/state/event 合同、recoverable exception taxonomy、stability sustained state |
| `tests/test_ekf.py` | 1 | EKF 基线 |
| `tests/test_event_schema.py` | 3 | event schema、counter-example registry、adapter exception cause fields |
| `tests/test_failure_trace.py` | 10 | §10 failure trace 连续性、窗口覆盖、full JSONL |
...
| `tests/test_stability_monitor.py` | 8 | Lyapunov stability monitor、manual-reset latch |
...
| §10 Failure Trace | `0 events / 0 kinds` | `14 events / 4 kinds` | 连续 stack 场景里的事件生命周期有可观测证据，且背景事件受控 |
...
| `analysis/artifacts/s10_trace_full.jsonl` | §10 全量 event trace，供审查覆盖率和 event line count |
...
- 11 个 runtime event kind：schema 有封闭注册和 counter-example。
- 依赖方向：`starship/` 不 import `sre_control/`，有测试守护。
- 剩余风险主要在 allocator fallback 语义、EKF hardening 与知识库入口漂移，不在“跑不起来”。
