...
1. [`CLAUDE_DEEP_REVIEW.md`](./CLAUDE_DEEP_REVIEW.md) §2：确认未发现 P0，逐条看 P1。
2. [`ENGINEERING_PACKET.md`](./ENGINEERING_PACKET.md) §4：对照 8 支柱、SRE adapter、event kind、测试。
3. [`CLAUDE_REVIEW_REQUEST.md`](./CLAUDE_REVIEW_REQUEST.md)：检查请求是否覆盖你的额外疑虑。
4. [`OPEN_RISKS.md`](./OPEN_RISKS.md)：把仍未修的项拆成下一批 PR。
...
请重点审查：Codex 是否把 Claude 上轮“事件生命周期只是叙事”的问题，真正推进成了可复现证据；以及新增的异常兜底 / stability guard 有没有把控制栈变得“看似稳健、实际吞错”。本轮深评的初步结论见 [`CLAUDE_DEEP_REVIEW.md`](./CLAUDE_DEEP_REVIEW.md)。
