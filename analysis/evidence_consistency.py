"""Study-level consistency checks used by the event evidence report."""
...
from analysis.evidence_artifacts import resolve_artifact
...
def _s10_background_event_fraction(trace_path: Path) -> float:
    ...
    event_ticks = {int(row["tick"]) for row in trace_rows if "tick" in row}
...
def _operator_actions_by_kind(rows: list[dict]) -> dict[str, list[str]]:
    ...
    return {kind: sorted(actions) for kind, actions in sorted(actions_by_kind.items())}
...
def _s12_replay_consistency(
    ...
    trace_rows = _jsonl_rows(trace_path)
    ...
    for field, expected_value in recovery.items():
        if diagnostics[field] != expected_value:
